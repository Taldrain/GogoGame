...
struct
  ...
  let string_of_color = function | White -> "white" | Black -> "black" | Empty -> ""
  
  let str_of_col = function | White -> "W" | Black -> "B" | Empty -> ""
...
struct
  ...
  let int_of_vertex bsize v = if v.pass then -1
    else bsize * (int_of_letter v.letter) + v.nb

  let vertex_of_int bsize i =
    let (q,r) = div_eucl i bsize in
      try {pass=false; letter=(letter_of_int q); nb=r }
      with Invalid_alphabet_of_int -> raise (Vertex_letter_error (string_of_int q))
end
